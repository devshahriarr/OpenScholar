...
export default async function PaperDetailsPage({
  ...
  let relatedData: any[] = [];
  let dbComments: any[] = [];
  let interactions = { isLiked: false, isSaved: false };
  ...
  try {
    ...
    paperData = await getPaperById(id);
    console.log(`[DEBUG] Paper data found: ${!!paperData}`);

    if (paperData) {
      // 2. Fetch Related Papers
      relatedData = await getRelatedPapers(id, paperData.categoryId);
      ...
      const { getUserProfile } = await import("@/modules/user/repository");
      
      const user = await getSessionUser();
      
      const [commentsResult, interactionsResult, profileResult] = await Promise.all([
        getPaperComments(id),
        user ? getUserInteractions(user.sub, id) : { isLiked: false, isSaved: false },
        getUserProfile(paperData.createdBy)
      ]);

      dbComments = commentsResult;
      interactions = interactionsResult;
      creatorProfile = profileResult;
      isFollowing = user ? await getIsFollowing(user.sub, paperData.createdBy) : false;
    }
  } catch (error) {
  ...
  const initialComments = dbComments.map(c => ({
    ...
    user: {
      ...
      avatarUrl: c.user?.profileImageUrl || undefined,
    },
    content: c.content,
    ...
    authors: [
      {
        ...
        institution: creatorProfile?.university || "OpenScholar University",
        bio: creatorProfile?.bio || "Research professional at OpenScholar.",
        followers: creatorProfile?.stats?.followers || 0,
        likes: creatorProfile?.stats?.likes || 0,
        avatarUrl: creatorProfile?.avatarUrl || undefined,
      }
    ],
